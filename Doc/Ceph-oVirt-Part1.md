...
HostedEngine хранилищем мы обеспечили, но виртуальным машинам отдавать образы дисков по NFS просто не рационально, потому как **Ceph->Ganesha->NFS** будет работать намного медленнее, чем отдать виртуальным машинам Ceph RBD напрямую (графики будут позже), вот тут нам уже поможет прослойка с Cinder. 
...
Запустим `df -h` или `mount |grep nfs`' проконтролируем, что том смонтирован и все сделано правильно.
...
Костин Алексей (Telegram: @rumanzo): помощь в связке Ceph + Ganesha + oVirt
